...
async function getOtherArticles($) {
	try {
		...
		otherArticles.each((i, item) => {
			...
			let date = $(item).children('h3').children('a').attr('title').trim().split('\r');
			// console.log(heading);
			// console.log(desc);
			// console.log(link);
			data.source = { id: null, name: 'The Hindu' };
			data.topic = 'national';
			data.section = 'otherArticles'
			data.subtopic = '';
			data.title = heading;
...
async function getMainArticles($) {
	try {
		...
		newsArticles.each((i, item) => {
			...
			let heading = $(item).children('.story-card-news').children('h3').children('a').text().trim();
			let date = $(item).children('.story-card-news').children('h3').children('a').attr('title');
...
async function getNationalData(url) {
	try {
		...
		// console.log(pages);
		let arr = await getOtherArticles($);
		let arr2 = await getMainArticles($);
